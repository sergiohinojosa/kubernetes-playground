...
readCredsFromFile() {
    if [ -f "$FILE" ]; then
        ...
        DT_INGEST_TOKEN=$(echo $CREDS | jq -r '.dynatraceIngestToken')
    fi
}
printVariables(){
    echo "Dynatrace Tenant (DT_TENANT): $DT_TENANT"
    echo "Dynatrace API Token (DT_API_TOKEN): $DT_API_TOKEN"
    echo "Dynatrace Ingest Token (DT_PAAS_TOKEN): $DT_INGEST_TOKEN"
...
writeCreadsToFile(){
    ...
    then
        ...
        jq --arg DT_INGEST_TOKEN "$DT_INGEST_TOKEN" '.dynatraceIngestToken = $DT_INGEST_TOKEN')
        CREDS=$NEWCREDS
        echo $CREDS > $FILE
        echo ""
        echo "The new credentials can be found here: $(pwd)/${FILE} "
        echo ""
    fi
}
...
readCredsFromTerminal(){
    ...
    read -p "Dynatrace Ingest Token (default=$DT_INGEST_TOKEN): " DT_INGEST_TOKEN_I
    echo ""
    ...
    if [ -n "${DT_INGEST_TOKEN_I}" ]; then
        DT_INGEST_TOKEN=$DT_INGEST_TOKEN_I
    fi
}
